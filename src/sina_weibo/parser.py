...
class ComWeibosParser(object):
    def __init__(self, uid, storage, **kwargs):
        self.uid     = uid
    ...
    def parse(self, pq_doc):
        ...
        def _parse_weibo(i):
            ...
            if txt is not None and (not txt.startswith(u'关注')):                
                ...
                self.weibos_storage.append(weibo['msgurl'])
...
class ComFollowsParser(object):
    ...
    def parse(self, pq_doc):
        def _parse_user(i):
            ...
            self.uids_storage.append(follow['uid'])
...
#             cnt = int(pq(pg).attr('action-data').split('&page=')[-1].strip())
...
class ComFansParser(object):
    ...
    def parse(self, pq_doc):
        def _parse_user(i):
            ...
            self.uids_storage.append(fan['uid'])
